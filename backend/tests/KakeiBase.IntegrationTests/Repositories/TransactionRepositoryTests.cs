...
public class TransactionRepositoryTests : IAsyncLifetime
{
    ...
    public async Task AddAsync_AndFindByIdAsync_ReturnsCreatedTransaction()
    {
        ...
        var transaction = Transaction.Create(user.Id, category.Id, 1000m, TransactionType.Expense, new DateOnly(2026, 7, 1), "ランチ");
    ...
    public async Task FindAllByUserIdAsync_ReturnsOnlyOwnTransactions()
    {
        ...
        var t1 = Transaction.Create(user.Id, category.Id, 1000m, TransactionType.Expense, new DateOnly(2026, 7, 1));
        var t2 = Transaction.Create(user.Id, category.Id, 2000m, TransactionType.Expense, new DateOnly(2026, 7, 2));
        var t3 = Transaction.Create(otherUser.Id, otherCategory.Id, 3000m, TransactionType.Expense, new DateOnly(2026, 7, 3));
    ...
    public async Task FindAllByUserIdAsync_WithYearMonthFilter_ReturnsOnlyMatchingTransactions()
    {
        ...
        var julyTx = Transaction.Create(user.Id, category.Id, 1000m, TransactionType.Expense, new DateOnly(2026, 7, 15));
        var juneTx = Transaction.Create(user.Id, category.Id, 2000m, TransactionType.Expense, new DateOnly(2026, 6, 10));
    ...
    public async Task FindAllByUserIdAsync_WithCategoryFilter_ReturnsOnlyMatchingTransactions()
    {
        ...
        var t1 = Transaction.Create(user.Id, category.Id, 1000m, TransactionType.Expense, new DateOnly(2026, 7, 1));
        var t2 = Transaction.Create(user.Id, otherCategory.Id, 500m, TransactionType.Expense, new DateOnly(2026, 7, 2));
    ...
    {
        var (user, category) = await CreateUserAndCategoryAsync();
        var transaction = Transaction.Create(user.Id, category.Id, 1000m, TransactionType.Expense, new DateOnly(2026, 7, 1));
