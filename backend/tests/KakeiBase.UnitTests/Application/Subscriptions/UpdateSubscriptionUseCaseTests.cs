...
public class UpdateSubscriptionUseCaseTests
{
    ...
    public async Task ExecuteAsync_WithValidUpdate_ReturnsUpdatedDto()
    {
        ...
        var originalCategoryId = subscription.CategoryId;

        _subscriptionRepository.FindByIdAsync(subscription.Id).Returns(subscription);

        var sut = CreateSut();
        var result = await sut.ExecuteAsync(userId, subscription.Id, "Disney+", 990, false);
        ...
        // カテゴリはシステム管理のため変更されないこと
        result.Subscription.CategoryId.Should().Be(originalCategoryId);
    ...
    public async Task ExecuteAsync_WithNonExistentId_ReturnsNotFound()
    {
        ...
        var sut = CreateSut();
        var result = await sut.ExecuteAsync(userId, subscriptionId, "Netflix", 1490, true);
    ...
    public async Task ExecuteAsync_WithOtherUsersSubscription_ReturnsNotFound()
    {
        ...
        var sut = CreateSut();
        var result = await sut.ExecuteAsync(userId, subscription.Id, "Netflix", 1490, true);
